...
class DjFriendService : Service() {
    ...
    private fun fetchSuggestions(artist: String, track: String) {
        serviceScope.launch {
            ...
            // Fallback A: artist.getSimilar → each artist's #1 top track
    ...
    private fun findLocalTrack(artist: String, track: String): Uri? {
        ...
        cursor.use {
            ...
            while (it.moveToNext()) {
                val candidate = FuzzyMatcher.normalize("${it.getString(artistIdx)} ${it.getString(titleIdx)}")
                ...
                if (dist < bestScore && dist <= threshold) {
                    ...
                    bestUri   = Uri.withAppendedPath(
                        MediaStore.Audio.Media.EXTERNAL_CONTENT_URI, it.getLong(idIdx).toString()
                    )
        return bestUri
    // ── Timeout ──────────────────────────────────────────────────────────────
    ...
    private fun updateNotification(statusText: String, suggestions: List<SuggestionResult>) {
        val bigText = if (suggestions.isEmpty()) "Searching Last.fm…"
            ...
            "${if (it.isLocal) "📁" else "🌐"}  ${it.track} by ${it.artist}"
        val builder = NotificationCompat.Builder(this, CHANNEL_ID)
            .setContentTitle("🎵  $currentTrack")
            .setContentText(statusText)
            .setStyle(NotificationCompat.BigTextStyle().bigText(bigText))
        suggestions.take(3).forEachIndexed { i, s ->
            val icon  = if (s.isLocal) android.R.drawable.ic_menu_save else android.R.drawable.ic_menu_search
            val label = "${if (s.isLocal) "📁" else "🌐"} ${s.track} – ${s.artist}"
